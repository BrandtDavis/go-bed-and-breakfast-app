...
func (m *postgresDBRepo) SearchAvailabilityByDatesByRoomID(start, end time.Time) (bool, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	query := `select count(id)
			  ...
			  where 
			    room_id = $1
			    and $1 < end_date and $3 > start_date;`
...
// SearchAvailabiltyForAllRooms returns a slice of available rooms, if any for given date range
